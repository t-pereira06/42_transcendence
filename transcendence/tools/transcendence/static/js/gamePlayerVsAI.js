async function PlayerVsAIPlayGame(event) {
	...
	try {
		...
		let padelsSize = { width: wallStroke, height: fieldHeight / 5, depth: gameDepth };
		...
		// Padels
		let leftPadel = createBox(padelsSize.width, padelsSize.height, padelsSize.depth, gameData.config.left_padel, -(fieldWidth - padelsSize.width) / 2, 0, padelsSize.depth / 2);
		let rightPadel = createBox(padelsSize.width, padelsSize.height, padelsSize.depth, gameData.config.right_padel, (fieldWidth - padelsSize.width) / 2, 0, padelsSize.depth / 2);
		...
		function gameResume(event, id) {
			closeModal(event, id);
			pause = false;
			updateScoreDiv(scoreHome, scoreAway);
...
async function PlayerVsAIConfigUser(event) {
	if (event === undefined || event === null) { return 1; }
	try {
		event.preventDefault();
	} catch (error) { return 1; }
	let csrftoken = Cookies.get('csrftoken');
	if (csrftoken === undefined || csrftoken === null) { return 1; }
	const response = await fetch(`${projectURL}/game/p-vs-ai-config-user/`, {
		method: 'POST',
		headers: { 'X-CSRFToken': csrftoken, },
	});
	const response_text = await response.text();
	...
	try {
		...
		await gameNavigate(event, 'p-vs-ai-config-game/');
		return 0;
	} catch (error) { return 1; }
}

async function PlayerVsAIConfigGame(event) {
	if (event === undefined || event === null) { return 1; }
	try {
		event.preventDefault();
	} catch (error) { return 1; }
	...
	const response = await fetch(`${projectURL}/game/p-vs-ai-config-game/`, {
		method: 'POST',
		headers: { 'X-CSRFToken': csrftoken, },
		body: form_data,
	});
	...
	if (!response.ok) {
		try {
			const json_data = JSON.parse(response_text);
			return showErrors(event, json_data, 'playervsaiconfiggame_error');
		} catch (error) { return 1; }
	}
	try {
		const json_data = JSON.parse(response_text);
		removeError('playervsaiconfiggame_error');
